...
function render(){
    ...
    for(let i = 0; i < taskList.length; i++){      /* taskList배열이 늘어날때, 그 길이 값마다, 반복식을 이용해 비워진 resultHTML공간을 다시 되풀이하여 채워줌  */
...
function filter(event){                // 포인터이벤트가 발생해 이벤트 내용에 div의 id 이름이 포함되어 있음 .target.id로 호출. 매개변수와 인수의 이름은 임의의 내용
